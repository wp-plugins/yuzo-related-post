    	...
    	$_html = "";
 		 ...
 		 if( $wp_query->post_count != 0 ){ // if have result in loop post
	    	 ...
	    	 if( is_array($post_type) ){
		    	 if( ! in_array($post_type, (array)$options_my_plugin[ $this->parameter['name_option'].'_post_type' ]) && ! is_search()  ){
		    	 	return;
		    	 }
		     }
  		...
  		if( have_posts() && $wp_query->post_count != 0 ){
  			...
  			if( (int)$options_my_plugin[ $this->parameter['name_option'].'_text2_length' ]>0 ){
  				$text2_extract = '<br /><span style="font-size:'.((int)$options_my_plugin[ $this->parameter['name_option'].'_font_size' ]-4).'px;" >'.self::yuzo_extract_title( strip_tags( get_the_content() ), (int)$options_my_plugin[ $this->parameter['name_option'].'_text2_length' ] ).'</span>';
  			}
