...
class yuzo_related_post{
	...
	function myoptions_build(){
		...
		$_options_type_post_new = array();
		foreach ( $post_types  as $key => $post_type ) {

			$id_post_type = "";
			$name_post_type = "";
			if( ! in_array( $post_type->name , array('revision','nav_menu_item') ) ){ // no include this type post
				$id_post_type = $post_type->name;
				$name_post_type = $post_type->labels->name;
			}

			if( $id_post_type && $name_post_type ){
				$_options_type_post_new[] = array('id'=>$this->parameter['name_option'].'_post_type',
											  	'value'=>$id_post_type,
											  	'text' =>__($name_post_type,$this->parameter['name_option']),
											  	'help' => '' );
			}
    	...
    	$_html = "";
 		 ...
 		 if( $wp_query->post_count != 0 ){ // if have result in loop post
			...
			$args = array('post__not_in' => $post__not_in,
						  ...
						  'tag__in' => $string_tags,
						  'post_type' => (array)$options_my_plugin[ $this->parameter['name_option'].'_post_type' ],
						  'post_status' => 'publish',
						  'caller_get_posts'=> 1,
