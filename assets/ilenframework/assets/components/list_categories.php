...
class ilenframework_component_list_category{
	...
	function display( $id_name , $seleted_array = array() ){			
		...
		foreach ($list_categories as $key2 => $value2): ?>
			...
			<div class="row_checkbox_list <?php if( $key2 != "-1"){ echo "checked_hidden"; } ?>" <?php if( $key2 != "-1"){ echo $style_check; } ?>>
				<input  type="checkbox" <?php if( in_array( $key2  ,  $seleted_array ) ){ echo " checked='checked' ";} ?> name="<?php echo $id_name ?>[]" id="<?php echo $id_name."_". $key2 ?>" value="<?php echo $key2; ?>" class="<?php if($key2=="-1"){ echo "check_all"; } ?>" />	&nbsp;<?php echo  $value2; ?> 
	...
	function load_script_and_styles(){
		//code 
		global $IF;
		if( is_admin()  && $_GET["page"] == $IF->parameter['id_menu'] ){
